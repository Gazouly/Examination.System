...
function checkPage(req, res, data) {
	...
	} else if (req.url === "/profile_staff.html") {
		sql = "Select * from staff ";
		req.url = '/profile.html';
	} else if (req.url === "/profile_employee.html") {
...
function checkExtension(req, res, fs, path) {
	if (req.url.match("\.del$")) {
		...
		else if (url.search('STAFF') != -1)
			req.url = '/profile_staff.html';
		else if (url.search('EMPLOYEE') != -1)
			req.url = '/profile_employee.html';
